		...
		while (i < loopSize) {
			if (i == loopSize2) {
				if (!sizeHash56) {
					...
					memcpy(&Dst[1], pSrc, remSize);
					memcpy(&Dst[1 + remSize / 4], Data, restSize);
					Dst[15] = (size * 2) | 1;
					pSrc = Dst;
				} else {
					DWORD remSize = size - processedSize;
					memcpy(&Dst[0], pSrc, remSize);
					memcpy(&Dst[remSize / 4], Data, 64 - remSize);
					pSrc = Dst;
				}
			} else if (i > loopSize2) {
				Dst[0] = size << 3;
		...
		vector<AssemblerParseError> *parse_errors)
{
	...
	memcpy(fourcc, pPosition, 4);
	pPosition += 4;
	memcpy(fHash, pPosition, 16);
	pPosition += 16;
	one = *(DWORD*)pPosition;
	pPosition += 4;
	fSize = *(DWORD*)pPosition;
	...
	pPosition += 4;
	chunkOffsets.resize(numChunks);
	memcpy(chunkOffsets.data(), pPosition, 4 * numChunks);
