                                               ...
                                               DWORD stride)
{
    ...
    pVertexStreamZeroData = (void*)((DWORD)pVertexStreamZeroData+sizeof(float)*2);
...
HRESULT _D3D8DrawRect(LPDIRECT3DDEVICE8 d, LPDIRECT3DTEXTURE8 texture,
                      RECT * rect, DWORD pix, float src[4])
{
    //#pragma comment(linker, "/EXPORT:" __FUNCTION__ "=" __FUNCDNAME__)
    ...
    d->SetRenderState(D3DRS_CULLMODE, 0);
...
ReleaseTexture8Hook(LPVOID texture)
{
    ...
    if (ref == 0)
    {
        ...
        if (t) D3D8HooksData->Surfaces->remove(t->level0);
...
HRESULT STDMETHODCALLTYPE
Release_Surface8Hook(LPVOID pSurface)
{
    #pragma comment(linker, "/EXPORT:" __FUNCTION__ "=" __FUNCDNAME__)
    if (D3D8HooksData->SingleCoreAffinity)       SetThreadAffinity(-1);
    HRESULT ref = D3D8HooksData->pIDirect3D_Surface8_Release(pSurface);
    if (ref == 0)
    {
        TEXTSWAP8_SURFACE * s = (TEXTSWAP8_SURFACE *)D3D8HooksData->Surfaces->Value(pSurface);
        if (s) RemoveTexture(s->ParentTexture);
        ...
        if (rep)
        {
            D3D8HooksData->pIDirect3D_Surface8_Release(rep);
            D3D8HooksData->Render_targets->discard(s);
        }
    }
    return ref;
                            ...
                            CONST RECT* pRect, DWORD Flags)
{
    #pragma comment(linker, "/EXPORT:" __FUNCTION__ "=" __FUNCDNAME__)
    HRESULT err;
    err = D3D8HooksData->pIDirect3DMSurface8_LockRect(surface,pLockedRect,pRect,Flags);
    if ( (err == 0) && (pRect == NULL) )
    {
        TEXTSWAP8_SURFACE * s =
...
HRESULT STDMETHODCALLTYPE
LockSurface8RectHookCheck(LPVOID surface, D3DLOCKED_RECT* pLockedRect,
                                 CONST RECT* pRect, DWORD Flags)
{
    #pragma comment(linker, "/EXPORT:" __FUNCTION__ "=" __FUNCDNAME__)
    HRESULT err;
    ...
    if (s)
    {
        /*
        LPVOID rep;
        if (rep = D3D8HooksData->Replacements->Value(s->ParentTexture))
        {
            D3D8HooksData->pIDirect3DTexture8_Release(rep);
            D3D8HooksData->Replacements->discard(s->ParentTexture);
        }
        */
       D3D8FreeReplacement((IDirect3DBaseTexture8*)s->ParentTexture);
    }
    return LockSurface8RectHook(surface, pLockedRect, pRect, Flags);
}
...
UnLockSurface8RectHook(LPVOID surface)
{
    ...
    if (t)
    {
        ...
        } else if (D3D8HooksData->pDumpTexture)
        {
            t->pData  = NULL;
            t->pitch  = 0;
            HRESULT r = D3D8HooksData->pIDirect3DMSurface8_UnlockRect(surface);
    ...
    return D3D8HooksData->pIDirect3DMSurface8_UnlockRect(surface);
}
