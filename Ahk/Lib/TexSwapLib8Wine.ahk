...
TS8Init(byref cfg)
{
	...
	if (g_.cfg.common_path)
		g_tswap8.path := g_.cfg.Mydocs "\Games\Peixoto\Textures" 
...
BrowseTextures8(pDevice, clr)
{
	...
	.  level0 ", active surfaces: " g_tswap8.Surfaces.count(), uint, g_tswap8.f_rct[])
...
{
	fileexist(f:=strget(index+0, "CP0")) ?: f := g_.cfg.MyDocs "\Games\checkerboard.dds"	
