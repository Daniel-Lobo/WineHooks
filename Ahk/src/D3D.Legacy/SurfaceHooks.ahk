SetupPrxs7(dd7, prim="")		
{
	...
	else
	{
		...
		for k, v in strsplit("prim dev 32bit s32bit vid cmp cpy D3D D3DZ", " ")
		g_.proxies[v] := ""	
		SetUpD3D(dd7)		
...
SetUpD3D(DD)
{
	g_.proxies.D3D   := (g_.cfg.D3D = 7) ? new D3DDevice7(DD) : new D3DDevice(DD)		
	g_.proxies.D3DZ  := (g_.cfg.D3D = 7) ? new D3DDevice7(DD,0x00010001,,1) : new D3DDevice(DD,0x00010001,,1)		
	;SetUPShaders()
}
...
SetUPShaders()
{
	...
	g_.proxies.xbr   := D3DDDI_CreatePixelShader(shader, "main_fragment")
	fileread, shader, % g_.cfg.injector_dir "\Shaders\xBRLv3.fx"
	g_.proxies.xbra  := D3DDDI_CreatePixelShader("#define USEALPHA 1`n" shader, "main")
	...
	g_.proxies.srt   := D3DDDI_CreatePixelShader(G_D3DDDI.shader_code, "Sort")	
	g_.proxies.srtr  := D3DDDI_CreatePixelShader(G_D3DDDI.shader_code, "Sorter")	
	g_.proxies.clr   := D3DDDI_CreatePixelShader(G_D3DDDI.shader_code, "Clear")	
	g_.proxies.ckey   := D3DDDI_CreatePixelShader(G_D3DDDI.shader_code, "ckey")	
	logerr(g_.proxies.clr)
	*/
	g_.proxies.xbr   := D3DDDI_CmplShder(g_.BCode.xbr)
	;g_.proxies.xbra  := D3DDDI_CmplShder(g_.BCode.xbra)
	g_.proxies.fmvs  := D3DDDI_CmplShder(g_.BCode.fmvs)
	g_.proxies.lnr   := D3DDDI_CmplShder(g_.BCode.lnr)
	g_.proxies.MIE   := D3DDDI_CmplShder(g_.BCode.MIE)
	g_.proxies.DIE   := D3DDDI_CmplShder(g_.BCode.DIE)
	g_.proxies.clr   := D3DDDI_CmplShder(g_.BCode.clr)
	g_.proxies.ckey  := D3DDDI_CmplShder(g_.BCode.ckey)
}
...
{
	fgs := [DDBLT_KEYSRCOVERRIDE, DDBLT_COLORFILL][f] | DDBLT_WAIT ;| DDBLT_DDFX 
...
GetRTD3D(rct, cpy=0)
{
	src := rect[]			
	rct := g_.proxies.FMVr[]			
	t   := new TxlSz(D3DHOOKS_DATA.W*D3DHOOKS_DATA.scale, D3DHOOKS_DATA.H*D3DHOOKS_DATA.scale)	
	
	D3DDDI_SetLinearFilter(True)	
	if (g_.cfg.sys)
	{		
		g_.proxies.D3D.blt(g_.proxies.vid, rct, g_.proxies.flp.texture, src)
		DDBlt(g_.proxies.dev.Surface, rct, g_.proxies.vid.Surface, rct, 0, g_HD.DDBLTFX[])		
	} else
		g_.proxies.D3D.blt(g_.proxies.dev, rct, g_.proxies.flp.texture, src)
	
	t := ""	
	if (cpy)
		g_.proxies.D3D.blt(g_.proxies.cpy, g_.proxies.FMVr[], g_.proxies.dev.texture, 0)
	D3DDDI_SetLinearFilter(False)
...
GetRT(rct=0, cpy=0)
{
	...
	if (g_.cfg.sys)
	{
		DDBlt(g_.proxies.vid.surface, 0, g_.rtrgt, src, 0, g_HD.DDBLTFX[])
		DDBlt(g_.proxies.dev.Surface, dst, g_.proxies.vid.Surface, dst, 0, g_HD.DDBLTFX[])			
	} else 
		DDBlt(g_.proxies.dev.surface, dst, g_.rtrgt, src, 0, g_HD.DDBLTFX[])
	if (cpy)
		DDBlt(g_.proxies.cpy.surface, dst, g_.proxies.dev.surface, dst, 0, g_HD.DDBLTFX[])		
}
...
UpdateRT(pRECT, cpy=False)
{
	...
	if (cpy)
	{
		g_.proxies.D3DZ.SetTexture(1, g_.proxies.cpy.Texture)	
		g_.proxies.D3DZ.blt(g_.proxies.flp, rect[], s.Texture, g_.proxies.FMVr[], g_.proxies.DIE)
		;return				 
	} else
		g_.proxies.D3DZ.blt(g_.proxies.flp, rect[], s.Texture, g_.proxies.FMVr[], g_.proxies.ckey)
	
	;g_.proxies.d3d.blt(s, g_.proxies.FMVr[], 0, 0, g_.proxies.clr)	
	;clear with a blt avoids a bizzare viewport bug on the phantom menace's load screens
...
Surface1UpDateRT(pRECT="", cpy=False)
{
	...
	{
		D3DDDI_SetLinearFilter(False)	
		g_.proxies.D3D.SetTexture(1, g_.proxies.cpy.Texture)
		g_.proxies.D3D.blt(g_.proxies.cmp, pRECT, s.Texture, pRECT, g_.proxies.MIE)
		g_.proxies.D3D.SetTexture(1, 0)	
		g_.proxies.D3D.trgt()
		DDBlt(g_.rtrgt, dst, g_.proxies.cmp.Surface, src, 1, g_HD.DDBLTFX[])	
		D3DDDI_SetLinearFilter(False)
	} else	
		DDBlt(g_.rtrgt, dst, s.Surface, src, 1, g_HD.DDBLTFX[])
...
{
	if (g_.cfg.NEFS || g_.cfg.SSAA)
	return dllcall(IDirectDrawSurface.blt, uint, g_.proxies.prim.surface, uint, 0, uint, g_.proxies.flp.flip
								         , uint, RECT[], uint, 0, uint, 0, uint)	
...
Surface1UpDatePrimB(p, pRECT)
{
	...
	if (g_.cfg.Blt3D)
	{
		;g_.proxies.D3D.blt({"surface4" : g_.proxies.flp.flip4}, rect[], g_.proxies.prim.Texture, g_.proxies.FMVr[], g_.proxies.ckey)
        g_.proxies.D3D.blt({"surface4" : g_.proxies.flp.flip4, "surface7" : g_.proxies.flp.flip}, rect[], g_.proxies.prim.Texture, g_.proxies.FMVr[], g_.proxies.ckey)   
		dllcall(IDirectDrawSurface.blt, uint, p, uint, _RECT[], uint, g_.proxies.flp.flip, uint, RECT[]
									  , uint, DDBLT_WAIT, uint, g_HD.DDBLTFX[], uint)	
	    dllcall(IDirectDrawSurface.blt, uint, g_.proxies.prim.Surface, uint, 0, uint, 0, uint, 0
									  , uint, DDBLT_COLORFILL, uint, g_HD.DDBLTFX[], uint)	
		return
	}		
	if (g_.cfg.NEFS || g_.cfg.SSAA)
	{
		dllcall(IDirectDrawSurface.blt, uint, g_.proxies.flp.flip, uint, RECT[], uint, g_.proxies.prim.Surface, uint, 0 
									  , uint, DDBLT_KEYSRCOVERRIDE, uint, g_HD.DDBLTFX[], uint)
		dllcall(IDirectDrawSurface.blt, uint, p, uint, _RECT[], uint, g_.proxies.flp.flip, uint, RECT[]
									  , uint, DDBLT_WAIT, uint, g_HD.DDBLTFX[], uint)					  
	    dllcall(IDirectDrawSurface.blt, uint, g_.proxies.prim.Surface, uint, 0, uint, 0, uint, 0
									  , uint, DDBLT_COLORFILL, uint, g_HD.DDBLTFX[], uint)	
	...
	dllcall(IDirectDrawSurface.blt, uint, g_.proxies.D3D.surface, uint, _RECT[], uint, p, uint, _RECT[] 
	  						      , uint, DDBLT_WAIT, uint, 0, uint)
	dllcall(IDirectDrawSurface.blt, uint, g_.proxies.D3D.surface, uint, _RECT[], uint, g_.proxies.prim.Surface, uint, 0 
								  , uint, DDBLT_KEYSRCOVERRIDE, uint, g_HD.DDBLTFX[], uint)
	dllcall(IDirectDrawSurface.blt, uint, p, uint, _RECT[], uint, g_.proxies.D3D.surface, uint, _RECT[] 
								  , uint, DDBLT_WAIT, uint, 0, uint)							   
	dllcall(IDirectdrawSurface.blt, uint, g_.proxies.prim.Surface, uint, 0, uint, 0, uint, 0
								  , uint, DDBLT_COLORFILL, uint, g_HD.DDBLTFX[], uint)	
	return 	
...
Surface1UpDatePrim(p, pRECT)
{
	...
	if (g_.proxies.fmv)
	{
		g_.proxies.D3D.draw(g_.proxies.fmvr[], src.Texture, 0, g_.proxies.fmvs)		
		dllcall(IDirectDrawSurface.blt, uint, p, uint, _RECT[], uint, g_.proxies.D3D.surface
		, uint, g_.proxies.fmvr[], uint, 0, uint, 0, uint)		
		return
	} else if (g_.cfg.xBR or g_.cfg.SCLR)
	{	
		if g_.cfg.SCLR
		{
			g_.proxies.D3D.draw(g_HD._Sclr.dst, src.Texture, g_HD._Sclr.src, (g_.cfg.xBR) ? g_.proxies.xbr : 0)
			DDWait(p)				
			dllcall(IDirectDrawSurface.blt, uint, p, uint, g_HD._Sclr.dst, uint, g_.proxies.D3D.surface
			, uint, g_HD._Sclr.dst, uint, 0, uint, 0, uint)
		} else {
			g_.proxies.D3D.draw(_RECT[], src.Texture, 0, (g_.cfg.xBR) ? g_.proxies.xbr : 0)	
			DDWait(p)	
			dllcall(IDirectDrawSurface.blt, uint, p, uint, _RECT[], uint, g_.proxies.D3D.surface
			, uint, _RECT[], uint, 0, uint, 0, uint)
		}		
		return	
	}	
	DDWait(p)	
