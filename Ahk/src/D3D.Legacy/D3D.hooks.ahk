...
#include <DirectX\lib\HDHooks>
#include <DirectX\lib\_TexSwapLib>
#include <DirectX\lib\TextureManager>
...
SSAA(byref sz, dd, fmt)
{
	...
	g_.proxies.D3Dssaa   :=  new D3DDevice(DD, (w<<16) |h)		
	...
	dllcall("peixoto.dll\Scaler", uint, D3DHOOKS_DATA.w, uint, D3DHOOKS_DATA.h, uint, w
	                            , uint, h, uint, 0, uint, g_HD.Sclr[]) 	
	if (g_.cfg.SSAA = 4)
...
{    
	...
	if (g_.cfg.SSAA)	{
		...
		(g_.proxies.flp.handle) ?: g_.proxies.flp.GetHandle(g_.proxies.d3d.device2)
		if (g_.cfg.SSAA=4)	
		{
			(g_.proxies.hlf.handle) ?: g_.proxies.hlf.GetHandle(g_.proxies.d3d.device2)
			g_.proxies.d3d.Trgt(g_.proxies.hlf)
			g_.proxies.d3d.draw(hRECT[], g_.proxies.flp.texture, 0, g_.proxies.lnr, 16)	
			g_.proxies.d3d.Trgt()
			g_.proxies.d3d.draw(_RECT[], g_.proxies.hlf.texture, 0, g_.proxies.lnr, 32)				
		} else {
			g_.proxies.d3d.draw(_RECT[], g_.proxies.flp.texture, 0, (g_.cfg.SSAA=1) ? 0 : g_.proxies.lnr, 16) 
		}
		DDWait(p1, p2&DDFLIP_WAIT)	
		dllcall(IDirectdrawSurface.blt, uint, p1, uint, 0, uint, g_.proxies.d3d.surface, uint, 0, uint, 0, uint, DDBLTFX[], uint)
		dllcall(IDirectdrawSurface.blt, uint, g_.proxies.flp.flip, uint, 0, uint, g_.rtrgt, uint, 0, uint, 0, uint, DDBLTFX[], uint)	
		if (g_.cfg.cpy)
		{
			if (g_.cfg.BLT3D)
			g_.proxies.D3DZ.blt(g_.proxies.flp, rect[], g_.proxies.cpy.Texture, g_.proxies.FMVr[])
			else DDBlt(g_.rtrgt, RECT[], g_.proxies.cpy.surface, 0, 0, g_HD.DDBLTFX[])
		}		
		return
	}
	else if (g_.cfg.NEFS)
	{
		DDWait(p1, p2&DDFLIP_WAIT)
		r := dllcall(IDirectdrawSurface.blt, uint, p1, uint, 0, uint, g_.rtrgt, uint, 0
		, uint, 0, uint, DDBLTFX[], uint)
		dllcall(IDirectDrawSurface.flip, uint, g_.rtrgt, uint, 0, uint, 0)	
		if (g_.cfg.cpy)
		dllcall(IDirectDrawSurface.blt, uint, g_.rtrgt, uint, RECT[], uint, g_.proxies.cpy.surface, uint, 0 
									  , uint, 0, uint, g_HD.DDBLTFX[], uint)
		/* Fliping doesn't work on grim fandango, but blit does. If more games require it, may it the  default behaivior 
		 * dllcall(IDirectdrawSurface.blt, uint, g_.proxies.flp.flip, uint, 0, uint, g_.rtrgt, uint, 0, uint, 0, uint, DDBLTFX[], uint)	
		 */
		return r
	} 
...
IDirectDrawSurface7_flip(p1, p2, p3)
{
	...
	if (g_.cfg.SSAA)	{
		...
		;dllcall(IDirectdrawSurface7.blt, uint, g_.proxies.d3d.Surface, uint, 0, uint, 0, uint, 0
	...
	{
		DDWait(p1, p2&DDFLIP_WAIT)		
		r := dllcall(IDirectdrawSurface7.blt, uint, p1, uint, 0, uint, g_.rtrgt, uint, 0
		                                    , uint, 0, uint, g_HD.DDBLTFX[], uint)
		dllcall(IDirectdrawSurface7.flip, uint, g_.rtrgt, uint, p2, uint, p3)
		if (g_.cfg.cpy)
		dllcall(IDirectDrawSurface.blt, uint, g_.rtrgt, uint, RECT[], uint, g_.proxies.cpy.surface7, uint, 0 
									  , uint, 0, uint, g_HD.DDBLTFX[], uint)		
		return r
	}	
	DDWait(p1, p2&DDFLIP_WAIT)		
